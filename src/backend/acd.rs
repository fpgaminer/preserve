...
impl Backend for AcdBackend {
	fn block_exists(&mut self, id: &BlockId) -> Result<bool> {
		...
		if let Some(_) = try!(self.acd.find_path(None, &("/gbackup/blocks/".to_owned() + &dir1 + "/" + &dir2 + "/" + &block_id))) {
			Ok(true)
		} else {
			Ok(false)
		}
	...
	fn fetch_block(&mut self, id: &BlockId) -> Result<EncryptedBlock> {
		...
		let acd_id = match try!(self.acd.find_path(None, &("/gbackup/blocks/".to_owned() + dir1 + "/" + dir2 + "/" + &block_id))) {
			Some(id) => id,
	...
	fn fetch_archive(&mut self, name: &EncryptedArchiveName) -> Result<EncryptedArchive> {
		let acd_id = match try!(self.acd.find_path(None, Path::new("/gbackup/archives/").join(name.to_string()))) {
			Some(id) => id,
