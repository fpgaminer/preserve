...
pub struct File {
    ...
    /// File mode (right now, it should just be permissions, since only directories and
    /// regular files are archived)
    pub mode: u32,
...
/// While the original filesystem was likely a file tree, we squash it to a simple list,
/// since dealing
/// with it as a tree would require lots of extra, nasty code.
/// The list is ordered such that folders are listed before the children inside of them,
/// making restore
/// easy.
...
impl Archive {
    pub fn encrypt(self, keystore: &KeyStore) -> Result<(EncryptedArchiveName, EncryptedArchive)> {
        ...
        let compressed = lzma::compress(encoded.to_string().as_bytes(), 9 | lzma::EXTREME_PRESET)
            .expect("internal error");  // Compression shouldn't fail
        let encrypted_archive = keystore.encrypt_archive(&encrypted_name, &compressed);

        Ok((encrypted_name, encrypted_archive))
    }

    pub fn decrypt(encrypted_name: &EncryptedArchiveName,
                   encrypted_archive: &EncryptedArchive,
                   keystore: &KeyStore)
                   -> Result<Archive> {
        let compressed = try!(keystore.decrypt_archive(encrypted_name, encrypted_archive));

        let decompressed = try!(lzma::decompress(&compressed)
            .map_err(|_| Error::CorruptArchiveFailedDecompression));
